...
const CONFIG = {
  API_URL: 'https://citrine-backend-production-94e0.up.railway.app/api',
...
function initAuth() {
  ...
  const showStep = (id) => {
    ['step1', 'step2', 'stepPasswordLogin'].forEach((s) => {
  ...
  try {
    ...
    if (data.banners && data.banners.length > 0) {
      ...
      if (bannerSlider) {
        ...
        document.getElementById('bannerImg').src = b.image_url.startsWith('http') ? b.image_url : `${CONFIG.API_URL.replace(/\/api\/?$/, '')}${b.image_url}`;
...
// Populates the branch <select> on the checkout form. Runs once at page
// load (branches change rarely) rather than on every visit to the view.
let checkoutBranchesLoaded = false;
async function loadCheckoutBranches() {
  if (checkoutBranchesLoaded) return;
  const select = document.getElementById('custBranch');
  if (!select) return;
  try {
    const res = await fetch(`${CONFIG.API_URL}/branches/public`);
    const branches = await res.json();
    if (!res.ok || !Array.isArray(branches)) throw new Error('Could not load branches');
    select.innerHTML = '<option value="" disabled selected>Choose the branch you\'re ordering from…</option>'
      + branches.map((b) => `<option value="${b.id}">${b.name}${b.addressLine ? ' — ' + b.addressLine : ''}</option>`).join('');
    checkoutBranchesLoaded = true;
    // Only one branch open? Pick it automatically so guests don't have to think about it.
    if (branches.length === 1) select.value = branches[0].id;
  } catch (err) {
    select.innerHTML = '<option value="" disabled selected>Branches unavailable — please refresh</option>';
  }
}
...
function initCheckout() {
  ...
  document.getElementById('checkoutForm')?.addEventListener('submit', async (e) => {
    ...
    const branchId = document.getElementById('custBranch').value;
    if (!branchId) { alert('Please choose a branch before placing your order.'); return; }
